...
\midrule
PR \#22836 & head \code{9955ff9e6}; base \code{69f54be2a} & All 46 changed filenames, current discussions, Actions state, and exact line anchors.\\
\code{vscode-mssql dev/query} & \code{5be148b1e} & Related query/data-plane refactor history and already-merged platform seams.\\
\code{vscode-mssql dev/refactor} & ref absent at review time & No assumptions are made from a deleted branch; surviving notes and current/merged code are used instead.\\
...
\code{kburtram/notes main} & \code{a548c0b84} & Metadata substrate/cache/drift, language-service, STS2, and observability design intent.\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{enumerate}
  ...
  \item \textbf{Current GitHub Actions logs.} At the reviewed head, analysis, packaging, normalization, and platform launch checks are green; the aggregate build-and-test check is red. The unit runner reports 3,975 passing, 0 failing, and 12 skipped; the aggregate failure comes from SQL Projects unit exit code 1 and one of 17 VSIX smoke tests.
  ...
  \item \textbf{Missing regression proof.} A safety claim is not treated as validated when the current tests avoid the relevant branch---for example, an offline test with polling disabled and a guaranteed cache hit.
\end{enumerate}

\begin{statusbox}[title={Current review disposition}]
The design can remain the basis of the refactor, but the current head should receive changes before merge. The first six findings in \cref{sec:blockers} affect truth, isolation, or retry ownership rather than optional polish. Additional hardening findings are documented for the same PR or an explicitly scheduled follow-up.
\end{statusbox}
...
Database isolation & A requested key can publish only metadata proven to belong to that database; recovery must reopen a fresh session. & \gap{} Initial mismatch logs and continues, and the wrapper drops \code{recycle()}.\\
Freshness/offline & Consumers choose stale, validated, live, or network-forbidden behavior. & \gap{} Policy vocabulary exists, but store-level offline mode does not prevent backend resolution, cache-miss hydration, polling, refresh, or auxiliary work.\\
STS2 query ownership & One active query per physical session, including lazy auxiliary sections. & \gap{} Main hydration is serialized; different auxiliary sections can overlap on one cached session and receive \code{Sts2.Busy}.\\
Cache integrity & A payload is bound to its key, logical hash, and a globally ordered publication decision. & \gap{} Byte/shape defenses are strong, but key binding, logical-hash verification, and atomic cross-window authority remain incomplete.\\
Data-plane retry policy & Serverless wake/retry has one owner per open attempt. & \gap{} Top-level and public service-view paths both wrap the same open.\\
...
\clearpage
\begin{reviewbox}[title={Why the current gaps are blocking}]
...
\begin{tikzpicture}[node distance=8mm and 7mm]
  ...
  \draw[failarr] (acq.north) to[out=55,in=125]
    node[labelbox,above,text width=50mm]{current head resolves the service before cache/offline authorization}
    (live.north);

  \node[note,text width=122mm,below=9mm of hit,xshift=24mm] {\textbf{Dynamic rule.} Online $\rightarrow$ offline must prevent future poll, retry, refresh, validation, and auxiliary opens for existing entries. Offline $\rightarrow$ online needs an explicit refresh or governed re-arm rule.};
\end{tikzpicture}}
\caption{Target acquisition ordering and the current bypass. Offline is a network authorization boundary, not merely ``do not schedule the first background refresh.''}
\label{fig:networkgate}
\end{figure}

\begin{reviewbox}
At \code{metadataStore.ts:417}, the current head resolves the SQL Data Plane service before cache load and before the offline outcome is known. On a miss, \code{MetadataService.acquire()} immediately hydrates; on a hit it still starts polling unless the test/config happens to disable it. Explicit refresh, strict freshness, and auxiliary sections are also callable. The existing offline test seeds a guaranteed hit and uses \code{pollSeconds=0}, so it does not prove the advertised contract.
\end{reviewbox}
...
The resource split keeps ordinary metadata work away from user query sessions. It does \emph{not}, by itself, serialize different auxiliary sections: the auxiliary engine coalesces callers for the same section but allows two different section keys to open the same cached session concurrently. STS2 documents and tests one active query per connection; the second execute returns \code{Sts2.Busy}. A catalog-wide auxiliary lane is therefore required.
...
\begin{tikzpicture}[node distance=9mm and 8mm]
  ...
  \node[note,text width=80mm,below=6mm of reject,xshift=-13mm] {Current head logs a preliminary mismatch and continues; H0 then adopts the actual database as the baseline. The store wrapper also omits \code{recycle()}, so timeout/rename recovery can reuse the old physical session.};
\end{tikzpicture}}
...
\begin{reviewbox}
Two local changes are necessary but not sufficient: forward \code{recycle: () => inner.recycle()} through the wrapper, and fail closed when the authoritative H0 identity differs from the requested key under the catalog's comparison rule. Tests should cover a backend that ignores \code{params.database}, case-insensitive spelling variation, a true case-sensitive mismatch, rename drift, and watchdog recovery proving that the next operation uses a new session.
\end{reviewbox}
...
Auxiliary queries use sources separate from the main hydration lane, which is the correct resource boundary. The current auxiliary engine serializes only callers for the \emph{same} section. Different keys can issue overlapping queries through one cached STS2 session, so a shared FIFO promise lane must sit around \code{sessions.open()} plus \code{runMetadataQuery()}. Names returned by auxiliary sections remain outside diagnostic events.
...
\begin{tikzpicture}[node distance=7mm and 9mm]
  ...
  \node[note,text width=91mm,below=7mm of save] {\textbf{Target containment.} Main-lane work is serialized. A watchdog fences the operation epoch, recycles the wedged source, and prevents late mutation. At the current head, the store wrapper omits \code{recycle()}, and auxiliary sections need their own shared lane.};
\end{tikzpicture}}
...
\code{offlineSnapshot} & user-selected offline operation & Target: perform no backend resolution, SQL session open, poll, validation, refresh, or auxiliary query. Return disk/memory snapshot as stale with age, or unavailable if none exists. Current store-level offline wiring does not yet enforce this boundary.\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[node distance=5mm and 7mm]
  ...
  \node[cache,minimum width=39mm,below=of offline] (o1) {target: no network\\\scriptsize current store bypasses gate};
...
The current implementation is strong on physical bounds, gzip limits, shape/ownership validation inside the payload, content-addressed filenames, and policy intersection. It does not yet bind the manifest to the requested key or recompute the canonical \code{contentHash} after decode.
...
\code{readEntry(key)} structurally validates the manifest but does not compare \code{manifest.key.serverFingerprint}, \code{databaseHash}, or \code{databaseExact} with the requested key. A valid entry copied into another hashed directory can therefore be served under the second key. Centralize a \code{manifestMatchesKey()} check and use it in both manifest-only and full-entry reads; treat mismatch as a distinct safe miss/quarantine reason.
\end{reviewbox}
...
\begin{tikzpicture}[node distance=8mm and 12mm]
  ...
  \node[note,text width=76mm,below=5mm of miss] {Current head implements strong byte, shape, and policy checks, but omits requested-key equality and recomputation of the decoded canonical content hash.};
\end{tikzpicture}}
...
\begin{tikzpicture}[node distance=9mm and 12mm]
  ...
  \node[note,text width=140mm,below=12mm of b2,xshift=20mm] {A's ``newer exists'' check happened before B published, so A can still install last and report success. A stale manifest-only writer has a related race: verify an old payload, let B replace/delete it, then reinstall a manifest that points to the missing old file.};
\end{tikzpicture}}
\caption{Uncovered two-window interleaving. Content-addressed payloads prevent byte mixing but do not make the authority decision atomic or globally ordered.}
...
\toprule
\textbf{Property} & \textbf{Current mechanism} & \textbf{Required repair}\\
\midrule
...
Logical equality is trustworthy & Manifest \code{contentHash} is used for skip-save & Recompute canonical content hash after decode and compare before adoption/manifest-only equality decisions.\\
...
\caption{Metadata test inventory at \code{9955ff9e6}.}\label{tab:testinventory}\\
\toprule
...
\midrule
\endhead
\code{metadataCatalog} & 35 & H-ladder parsing, readiness/partial failures, resolution/search, DDL keyword detection, descriptions, lazy definitions, burst coalescing, hidden columns, safe IDs.\\
...
\code{metadataFreshness} & 8 & Four policy modes, TTL, timeout-as-race, section gates, and stale/live/unavailable honesty.\\
...
\code{metadataCacheStore} & 36 & Path containment, byte integrity, oversize bounds, quarantine, rename retry, torn writes, selected two-writer/read races, eligibility/skips, policy intersection, manifest-only save, privacy canaries, index/eviction/clear/settings.\\
\midrule
\textbf{Metadata subtotal} & \textbf{151} & Current GitHub Actions per-file counts at the pinned head; zero failures.\\
\bottomrule
\end{longtable}
...
\centering
\small
\caption{Regression matrix required to close the current review findings.}
\label{tab:regressions}
\begin{tabularx}{\textwidth}{@{}p{0.08\textwidth}p{0.28\textwidth}p{0.29\textwidth}X@{}}
\toprule
\textbf{ID} & \textbf{Invariant} & \textbf{Current coverage} & \textbf{Missing deterministic proof}\\
\midrule
F1 & Offline setting means zero live work & Guaranteed disk hit, \code{pollSeconds=0}, session count & Miss; default polling; resolver count; explicit refresh/strict freshness; auxiliary; online$\rightarrow$offline.\\
...
F4 & Cache entry belongs to requested key & Path derivation/containment & Copy valid A manifest+payload into B directory; B must miss/quarantine.\\
...
F10 & Manifest logical hash matches decoded payload & SHA and canonical hash unit tests separately & Valid compressed SHA/payload shape with incorrect manifest \code{contentHash}.\\
\bottomrule
\end{tabularx}
\end{table}
...
The current offline defect is directly relevant to this scenario: a benchmark can report a disk snapshot while still resolving live infrastructure or arming later polling. The probe should assert resolver calls and session opens are zero, not only that the returned source label is \code{disk}.
...
At \code{9955ff9e6}, analysis/CodeQL, Azure PR validation, packaging, normalization, CLA, and all six platform VSIX-launch jobs pass. The GitHub \code{build-and-test} check is red, but its metadata-relevant facts are more specific:
...
\centering
\small
\caption{Independent local validation snapshot, 28 August 2026.}
...
\midrule
vscode-mssql build & Pass & Full extension build completed on \code{9955ff9e6}.\\
...
\begin{evidencebox}
The local runs establish that the pinned trees build and that existing metadata, cache, STS2, and embedded-perftest suites pass. They do not close F1--F11: the required adversarial branches and interleavings are absent from the current tests, and the live provisioned warm-cache and on-prem SQL matrices were not run.
\end{evidencebox}
...
\begin{itemize}
  \item provider unavailability from a public SQL Data Plane view is classified as \code{capabilityUnsupported};
  \item the cache-entry QuickPick combines a fingerprint prefix with localized text in one \code{description} string.
\end{itemize}
...
\centering
\small
\caption{Load-bearing mechanisms, current evidence, and remaining closure condition.}
\label{tab:traceability}
\begin{tabularx}{\textwidth}{@{}p{0.24\textwidth}p{0.32\textwidth}X@{}}
\toprule
\textbf{Mechanism} & \textbf{Current evidence} & \textbf{Closure condition}\\
\midrule
...
Database identity & Requested database parameter, tripwire event, digest rider & Reject before publication and forward recycle; prove wrong-backend/rename/watchdog cases.\\
...
\begin{enumerate}
  \item resolve F1--F3 so offline, identity, and STS2 lane contracts are trustworthy;
...
\subsection{Current-head implementation plan}\label{sec:blockers}

\begin{table}[H]
\centering
\small
\caption{Recommended repair order for PR \#22836.}
\label{tab:repairorder}
\begin{tabularx}{\textwidth}{@{}p{0.08\textwidth}p{0.30\textwidth}X@{}}
\toprule
\textbf{Order} & \textbf{Repair} & \textbf{Why this order}\\
\midrule
...
The current head is not yet a safe merge point for that foundation. Several claims in the original design narrative---zero-network offline operation, key-correct publication, one-active-query auxiliary behavior, newer-writer cache authority, and one serverless wake policy---are target invariants rather than properties enforced by \code{9955ff9e6}. The repair plan is bounded and testable. Once those gaps are closed, the same design can remain intact; the work is primarily to make the implementation honor its own boundaries.
\end{takeawaybox}
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22836}{PR \#22836}, current body/discussions/check metadata, head \code{9955ff9e6}, base \code{69f54be2a}; all 46 changed filenames.\\
...
Implementation review & Companion workspace review \path{vscode_mssql_pr_22836_code_review.md}; every finding was rechecked against local head \code{9955ff9e6} and the live PR head before publication.\\
