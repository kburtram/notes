...
\textbf{Review outcome at the merge-ready head.} The architecture and implementation now agree on the load-bearing boundaries. The six prior merge blockers---offline network enforcement, fail-closed database identity plus session recycling, auxiliary-session serialization, cache-key binding, cross-window cache publication ordering, and single serverless-wake ownership---are repaired and covered by deterministic regressions. The branch now includes current \code{main}; activation-snapshot context keys also prevent reload-required preview settings from exposing unregistered commands. Local build, lint, focused tests, and direct packaging are green subject only to two independently reproduced, untouched Windows path-fixture suites.
\end{tcolorbox}

\vspace{0.20in}
\begin{tabular}{@{}ll@{}}
Prepared for: & Karl Burtram\\
Primary change: & \code{dev/karlb/query_04_metadata_service} at \code{37bf23fce}\\
Current main merged: & \code{59c8ea50c}; original extraction base \code{69f54be2a}\\
Related vscode ref: & \code{dev/query} at \code{5be148b1e}\\
STS2 reference: & \code{sqltoolsservice dev/query} at \code{fe8300939}\\
Refactor notes: & report source parent \code{581b926c}\\
...
\midrule
PR \#22836 & merge-ready head \code{37bf23fce}; current-main parent \code{59c8ea50c}; extraction base \code{69f54be2a} & Current changed filenames, review discussions, Actions state, and exact line anchors.\\
...
\begin{enumerate}
  ...
  \item \textbf{GitHub Actions logs.} At historical head \code{9955ff9e6}, analysis, packaging, normalization, and platform launch checks were green; the aggregate build-and-test check was red even though its MSSQL lane reported 3,975 passing, 0 failing, and 12 skipped. The aggregate failure came from SQL Projects unit exit code 1 and one of 17 VSIX smoke tests. At this report snapshot, fresh \code{37bf23fce} CodeQL, normalization, and CLA checks are green while build/test and VSIX launch packaging remain in progress; their conclusions are intentionally not predicted here.
...
\centering
\small
\caption{Regression matrix closing the review findings at merged head \code{37bf23fce}.}
...
This was not evidence of a metadata failure. The later correctness head became conflicting as \code{main} advanced. Merge commit \code{37bf23fce} incorporates current \code{main}, resolves that conflict, and triggered a fresh workflow. At this snapshot, CodeQL, normalization, and CLA are green while build/test and VSIX launch packaging remain in progress; live PR checks remain the authority for the eventual merge button.
...
\midrule
vscode-mssql build & Pass & Full monorepo build completed on \code{37bf23fce}: shared toolkit, MSSQL extension/typecheck/bundles/webviews/notebook renderer, merged SQL Projects runtime, legacy SQL Database Projects package, and Data Workspace.\\
vscode-mssql lint & Pass & Root lint exited zero for extension-toolkit, MSSQL, SQL Database Projects, and Data Workspace.\\
Merged-head focused tests & 260 pass; 0 fail & Metadata/cache/catalog, auxiliary, profile/preview/manifest, SQL Data Plane, and extension activation-context selection.\\
MSSQL broad hosted tests & 4,025 pass; 0 fail; 10 skip & Complete inverse selection excludes exactly \code{QueryCompletionSoundService} and \code{Utility Tests - Path handling}. The full run otherwise reports eight failures, all reproduced alone as untouched Windows path-separator expectations (seven sound, one tilde path).\\
...
All 50 review threads are resolved at the pinned head with source-attributed dispositions. Provider startup refusal is now retryable \code{unavailable}; the QuickPick keeps the localized capture label separate from fingerprint detail; and the final two Copilot findings are closed by activation-snapshot context keys. A reload-required setting can therefore be turned off and hide UI immediately, while turning it on cannot reveal commands until the reloaded host has registered their handlers.
...
\centering
\small
\caption{Completed repair and merge-closeout order for PR \#22836 at \code{37bf23fce}.}
...
At \code{37bf23fce}, the implementation enforces the design narrative's load-bearing claims: zero-network offline operation, key-correct publication, one-active-query auxiliary behavior, newer-writer cache authority, decoded logical integrity, bounded hydration recovery, one serverless wake owner, and activation-consistent preview UI. The feature remains private-preview/default-off and no ordinary language, AI, OE, or Query Studio surface is cut over by this PR.
\end{takeawaybox}
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22836}{PR \#22836}, current body/discussions/check metadata, merge-ready head \code{37bf23fce}, current-main parent \code{59c8ea50c}, extraction base \code{69f54be2a}.\\
...
Implementation review & Companion workspace review \path{vscode_mssql_pr_22836_code_review.md}; every finding was rechecked against local and pushed head \code{37bf23fce} before publication.\\
