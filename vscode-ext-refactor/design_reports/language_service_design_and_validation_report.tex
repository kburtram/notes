...
\textbf{Assessment at the reviewed head.} PR \#22860 establishes a coherent, source-only language and scripting foundation. Its dependency boundaries, pinned metadata seam, deterministic feature pipeline, stale-safe diagnostics scheduler, and strict scripting provenance are sound. Review commit \code{b66db2d8f} closes the original 26 threads and one immediate follow-up, including the eight reproduced ordinary-SQL trust defects, with focused regression coverage and a real catalog-builder/store/provider-to-engine path. Exact-head local validation passed 707 owned language/scripting tests, 47 adjacent tests, the 4,742-test broad MSSQL lane, SQL Projects, build, lint, formatting, hooks, and online packaging; all hosted checks are green and the packaged MSSQL VSIX has a zero-byte delta. A later review found one residual spaced alias-column-list variant, so automated closeout has one valid correctness thread remaining; consumer activation remains gated independently on preview composition and integration proof.
\end{tcolorbox}
...
Current \code{main}: & \code{c6d2baa442}; topic ahead 3 / behind 9\\
Metadata prerequisite: & merged PR \#22836 / shared metadata substrate\\
Focused corpus: & 707 owned / 754 total focused tests\\
...
\midrule
PR head & \code{b66db2d8fde1} & Exact source and local validation target reviewed in this report.\\
...
Change size & 59 files; +26,391 / -0 & A large but isolated source extraction, dominated by new language, scripting, and test files.\\
GitHub workflows & Build/test, CodeQL, normalization, and six-platform VSIX launch all green & Existing build, lint, hosted test, package, and launch lanes pass at the pinned head.\\
Package comparison & MSSQL 85,347 KiB on target and PR; 0 KiB delta & The new source is not an input to the shipped extension bundle.\\
...
Review state & 27 resolved threads; one later residual thread open & Original closeout is complete; the ordinary spaced alias-list variant is a valid final correctness repair.\\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[model,minimum width=31mm,right=of source] (focused) {Focused tests\\\scriptsize 707 owned / 754 total};
...
\begin{evidencebox}
The strongest current claims are architectural isolation, deterministic behavior on the focused corpus, package invisibility, and repair coverage for the original review findings. The weakest claim remains real-world semantic authority across the full T-SQL grammar. The external differential review improved the honesty corpus and then found one residual whitespace variant; that is exactly why activation requires a broader live differential corpus rather than relying on aggregate coverage alone.
\end{evidencebox}
...
Honest diagnostic suppression & \partialmark & The original eight reproduced legal-SQL defects are repaired; one spaced alias-column-list variant remains open.\\
...
Current-main merge readiness & \partialmark & The branch is mergeable and fully green, but one valid review repair remains and the topic is nine mainline commits behind.\\
\bottomrule
\end{tabularx}
\end{table}
...
The original differential review reproduced a statement-location fallback that bypassed comment suppression plus derived-alias, \code{UPDATE TOP}, \code{db..table}, temp-overlay, case-sensitive dedupe, and bare-\code{alias.} edit gaps. Review commit \code{b66db2d8f} repairs and regression-tests those cases. A later probe found that whitespace between a derived alias and its column list still bypasses the balance skip; that remaining variant is tracked explicitly in \cref{sec:tiera} and is not hidden behind the word ``preview.''
...
\centering
\small
\caption{Independent local validation at \code{b66db2d8f}; hosted conclusions refreshed on 31 August 2026.}
...
At report freeze, GitHub reports the PR open, mergeable, non-draft, labeled \code{Automated Review}, and still requiring human review. At head \code{b66db2d8f}, build-and-test, actions and JavaScript/TypeScript analysis, CodeQL, normalization, CLA, package creation, Azure \code{VSCode MSSQL - Pull Request} validation, and all six Linux/macOS/Windows x64/arm64 VSIX-launch jobs are green.
...
\begin{tikzpicture}[node distance=8mm and 9mm]
  ...
  \node[risk,minimum width=35mm,below=11mm of host] (copilot) {1 residual thread\\\scriptsize whitespace before alias column list};

  \node[note,below=8mm of copilot,minimum width=73mm] (total) {27 threads resolved; one later ordinary-formatting variant remains open.};
\end{tikzpicture}}
\caption{Review disposition map at \code{b66db2d8f}. The remaining red item is a narrow but valid semantic-trust repair.}
\label{fig:findings-map}
\end{figure}

\subsection{Review snapshot: original closeout plus one residual}

The pre-fix PR had 26 current review threads at \code{eafbbdc046}: two from Copilot and 24 from a later source-attributed Fable review posted through the PR author's account. Every initial thread received an explicit disposition and was resolved after review commit \code{b66db2d8f}. One immediate Copilot follow-up about the documented newline precondition of \code{withHeader} was dispositioned after a complete production call-site audit, producing 27 resolved threads. A later Fable probe then found one valid residual variant: whitespace between a derived alias and its column list prevents the current raw-token-index guard from seeing the opening parenthesis. At report freeze that new thread is the sole unresolved thread.
...
\begin{enumerate}[label=\textbf{A\arabic*.},leftmargin=2.2em,itemsep=5pt]
  ...
  \item \textbf{Derived/VALUES alias lists} (\path{sketch/index.ts}). Unspaced \code{v(a,b)} lists are balance-skipped so commas cannot create phantom sources. \gap{} A space or newline before \code{(a,b)} still leaves the guard on trivia; advance to the next significant token and add spaced sketch/honesty cases.
...
The original findings are resolved or explicitly dispositioned in the PR. The later spaced-alias-list reproduction remains a valid correctness repair. The fully green build/test matrix proves regression stability against the existing corpus; it does not negate a newly reproduced case outside that corpus.
...
The topic includes extraction base \code{95cf790e5}; the live PR API identifies base \code{53b707bbe}; and current repository \code{main} is \code{c6d2baa442}. The exact topic is three commits ahead of and nine commits behind current \code{main}. The mainline-only commits touch controller, document-service, package, test, and extension infrastructure. The PR remains mergeable and all hosted checks are green; every subsequent consumer must nevertheless start from refreshed \code{main} and repeat build, focused/broad semantics, package, launch, and review checks.

\section{Integration and graduation plan}

\subsection{Next implementation step}

The next PR should be a gated consumer, not another implicit activation edit scattered across the extension. Before that consumer is made reachable, the remaining spaced alias-list variant in \cref{sec:tiera} should be repaired and pinned by the honesty/suppression corpus. The consumer PR should then:
...
\midrule
Ordinary-SQL trust corpus & Nearly closed & Original eight cases are repaired; add significant-token handling and spaced/newline alias-list regressions for the remaining variant.\\
...
Completion determinism & Category/rank/ordinal tests; repaired suppression, dedupe, overlay, and edit-range cases & Close the spaced alias-list residual; add cross-platform replay.\\
Diagnostic honesty & T1/T2 split, expanded honesty suite, freshness/drift tests, and original Tier-A repairs & Pin the residual formatting variant and add a real-server/native-vs-STS differential corpus.\\
...
The later review materially strengthened the correctness disposition. The eight original ordinary-SQL trust defects now have direct repairs and regression cases; the broader cache-identity, completion-edit, provider, scripting-sanitization, static-data, and real-adapter gaps are closed or explicitly dispositioned. The router timeout is accurately documented as cooperative rather than preemptive for synchronous work. One residual ordinary-formatting variant remains: whitespace before a derived alias column list can still create a phantom source and false 208 diagnostic. Green CI proves the encoded corpus is stable; zero package delta proves current users cannot reach the code. Neither makes that newly reproduced case disappear.
\end{takeawaybox}

\begin{statusbox}[title={Decision statement}]
Recommended disposition at this snapshot: \textbf{accept the architecture and retain the source-only boundary, but close the one valid residual review thread before final automated sign-off}. Exact-head local and hosted validation is otherwise green, all prior threads are dispositioned, and the PR is mergeable subject to human review. A later consumer PR must independently satisfy the full private-preview umbrella/feature gates, invisible command/menu contribution, runtime fail-closed behavior, and integration-level scenario validation before exposing the engine through Query Studio, a VS Code provider, command, or AI context path.
\end{statusbox}
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22860}{PR \#22860}, head \code{b66db2d8f}, extraction base \code{95cf790e5}, live API base \code{53b707bbe}, 27 resolved threads plus one later unresolved residual, Copilot review, source-attributed Fable review and probe report, exact-head sign-off comment, fully green Actions/Azure rollup, package-size bot, and Codecov report.\\
