...
\begin{enumerate}
  ...
  \item \textbf{Lexer} recognizes T-SQL words, identifiers, punctuation, comments, strings, variables, SQLCMD-sensitive regions, and \code{GO} / \code{GO n} line semantics.
...
\begin{tikzpicture}[node distance=9mm and 11mm]
  ...
  \node[engine,minimum width=31mm,right=of elig] (native) {Native call\\\scriptsize timeout boundary};
...
At report freeze, GitHub reports the PR open, mergeable, non-draft, labeled \code{Automated Review}, and still requiring human review. Build-and-test, actions and JavaScript/TypeScript analysis, CodeQL, normalization, CLA, package creation, and all six Linux/macOS/Windows x64/arm64 VSIX-launch jobs are green. The separate Azure \code{VSCode MSSQL - Pull Request} context remains pending.
...
\subsection{Automated-review findings still open}

Copilot reviewed 46 of 59 files and left two unresolved threads at \code{eafbbdc046}:

\begin{enumerate}
  \item \path{sqlScripting/scriptWriter.ts} counts \code{\n} while tracking anchors but does not treat a standalone carriage return as a newline. LF and CRLF output are handled, but CR-only or mixed CR-only content could produce incorrect anchor line/character values. This is a narrow compatibility edge, not evidence that ordinary generated CRLF scripts are mis-anchored.
  \item \path{sqlScripting/scriptingService.ts} says synonym targets are ``not hydrated'' when the real limitation is that this engine has no scriptable synonym definition. The behavior is an honest refusal; the explanatory note is imprecise.
\end{enumerate}
...
\begin{itemize}
  ...
  \item Unit fixtures cover a wide semantic corpus, but real-server collation, permissions, catalog scale, dynamic SQL, uncommon grammar, Azure/serverless behavior, and remote extension-host performance remain preview validation work.
\end{itemize}
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22860}{PR \#22860}, head \code{eafbbdc046}, extraction base \code{95cf790e5}, live API base \code{53b707bbe}, review threads, Actions check rollup, package-size bot, and Codecov report.\\
