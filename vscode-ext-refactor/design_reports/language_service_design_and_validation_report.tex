...
\midrule
PR head & \code{2bc5811e3abf} & Exact source and targeted validation target reviewed in this report.\\
...
\begin{tikzpicture}[node distance=7mm and 7mm]
  ...
  \node[model,minimum width=31mm,right=of source] (focused) {Focused tests\\\scriptsize 708 owned / 715 targeted};
...
Current-main merge readiness & \partialmark & The branch is mergeable with zero unresolved threads; the final hosted rerun is in progress and the topic is nine mainline commits behind.\\
\bottomrule
\end{tabularx}
\end{table}
...
The original differential review reproduced a statement-location fallback that bypassed comment suppression plus derived-alias, \code{UPDATE TOP}, \code{db..table}, temp-overlay, case-sensitive dedupe, and bare-\code{alias.} edit gaps. Review commit \code{b66db2d8f} repairs and regression-tests those cases. A later probe found that whitespace between a derived alias and its column list still bypassed the balance skip; final commit \code{2bc5811e3} advances through trivia, captures the alias list, and applies its positional column renames in the binder. Spaced and newline-separated forms now have sketch and diagnostic-honesty coverage.
...
\midrule
\textbf{Total} & \textbf{708} & \textbf{Zero failures and zero skips in the owned focused selection.}\\
\bottomrule
\end{longtable}

\subsection{Exact-head local validation and observed CI}

The full matrix was run at review commit \code{b66db2d8f}; final residual commit \code{2bc5811e3} then received targeted exact-head validation over its owning language/sketch/binder surface plus observability:

\begin{table}[H]
\centering
\small
\caption{Independent local validation through final head \code{2bc5811e3}; hosted final-head rerun in progress.}
...
Owned language/scripting at final head & 708 pass; 0 fail & All files in \cref{tab:tests}; spaced/newline alias lists and positional renames included.\\
...
Production online package & Pass at pre-final head & Released STS \code{6.0.20260827.1}; local artifact 1,542 files / 104.9 MB. Metafile contains no new language/scripting bundle input; final hosted package rerun is pending.\\
\bottomrule
\end{tabularx}
\end{table}
...
The eight original Tier-A findings were not architectural failures, but they were the exact user-trust failure class the diagnostic honesty design is intended to prevent. All eight have direct repairs, and final head \code{2bc5811e3} closes the last formatting variant of the fifth repair.
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22860}{PR \#22860}, head \code{2bc5811e3}, extraction base \code{95cf790e5}, live API base \code{53b707bbe}, 28 dispositioned threads / zero unresolved, Copilot review, source-attributed Fable reviews and probe reports, the pre-final sign-off comment, final-head targeted validation, hosted rerun state, package-size bot, and Codecov report.\\
