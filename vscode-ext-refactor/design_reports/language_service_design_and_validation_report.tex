...
\textbf{Assessment at the reviewed head.} PR \#22860 establishes a coherent source-only language and scripting library. Its contracts are VS Code-neutral; requests pin one immutable metadata view; completion and hover can request background hydration without awaiting it; and diagnostics distinguish syntax truth from metadata-dependent claims. The branch adds no commands, settings, provider registration, controller, activation path, or extension bundle input. The focused 675-test corpus, full local build/lint, broad hosted suites, Azure validation, and all six platform launch checks pass. A later Fable review nevertheless reproduced eight ordinary-SQL false-positive/suppression defects plus fifteen Tier-B hardening issues and one static-catalog error; together with two Copilot threads, all 26 review threads remain unresolved. The architecture is strong, but semantic closeout is not complete and the library must not be exposed to users in this state.
\end{tcolorbox}

\vspace{0.18in}
\begin{tabular}{@{}ll@{}}
Prepared for: & Karl Burtram\\
Primary change: & \code{dev/karlb/query_05_language_service} at \code{eafbbdc046}\\
Extraction base: & \code{95cf790e5}; 59 files, +25,750 / -0\\
...
Report source parent: & \code{bba79742e} in \code{kburtram/notes main}\\
...
\begin{enumerate}
  ...
  \item \textbf{Lexer} recognizes T-SQL words, identifiers, punctuation, comments, strings, variables, SQLCMD-sensitive regions, and \code{GO} / \code{GO n} line semantics. The current review found two boundary gaps: Unicode non-breaking space classification and a construct opener at exact EOF.
...
At the pinned head, the architecture and most contexts follow that policy, but the review reproduced a statement-location fallback that bypasses comment suppression and additional derived-alias, \code{UPDATE TOP}, \code{db..table}, temp-overlay, case-sensitive dedupe, and bare-\code{alias.} edit gaps. These are tracked explicitly in \cref{sec:tiera,tab:tierbc}; they are not limitations to hide behind the word ``preview.''
...
Diagnostics & 168 & Lexical, \code{GO}, structural and recovery T1; 208/207/209 T2; 62-case zero-unexpected-diagnostic honesty corpus; suppression accounting; lazy hydration; engine pass.\\
...
\midrule
\textbf{Total} & \textbf{675} & \textbf{Zero failures and zero skips in the focused selection.}\\
\bottomrule
\end{longtable}

\subsection{Broader local validation}

Validation was run after merging extraction-base \code{95cf790e5} into the topic branch:

\begin{table}[H]
\centering
\small
\caption{Independent local validation at \code{eafbbdc046}.}
...
Focused language/scripting & 675 pass; 0 fail & All 77 suites in \cref{tab:tests}; no skipped cases.\\
Observability contracts & 7 pass; 0 fail & Prefix classification and accepted diagnostics registry behavior.\\
Broad MSSQL hosted tests & 4,710 pass; 0 fail; 10 skip & Inverse selection excludes only two independently documented current-main Windows baseline suites: \code{Utility Tests - Path handling} and \code{QueryCompletionSoundService}.\\
...
Production online package & Pass & Released STS \code{6.0.20260827.1}; local artifact 1,542 files / 99.45 MiB. Metafile contains no new language/scripting bundle input.\\
\bottomrule
\end{tabularx}
\end{table}
...
The Fable review added useful \reported{} evidence beyond the fixture suites. It compiled the exact head, ran the 675 language/scripting tests plus seven observability tests (682/682), hydrated the real PR04 \code{MetadataStore} with a 10,201-object B16 fixture, adapted it through \code{CatalogLanguageMetadataProvider}, and exercised \code{NativeSqlLanguageEngine}. It also ran malformed-input/depth probes and compared the curated system catalog with a live SQL Server 2025 instance. These results were independently posted by the reviewer; they were not rerun in this report-writing pass.

\begin{table}[H]
\centering
\small
\caption{Fable review-machine measurements at \code{eafbbdc046}; observations, not product SLAs.}
...
Malformed/depth sweep & No throws; 5,000 parens in 13 ms & Supports total/tolerant direction, while the review still found an opener-at-EOF token-coverage hole.\\
\bottomrule
\end{tabularx}
\end{table}

The same live-catalog comparison found that all 157 curated built-in function names were genuine, but the two \code{dm_exec_sql_text} / \code{dm_exec_query_plan} entries model input handle names as result columns. Their real useful outputs include \code{dbid}, \code{objectid}, \code{number}, \code{encrypted}, and \code{text} or \code{query_plan}. The review's real-store path worked, but CI still uses fixture providers or hand-built handles rather than an end-to-end \code{CatalogBuilder}/store snapshot suite.

\subsection{GitHub validation snapshot}

At report freeze, GitHub reports the PR open, mergeable, non-draft, labeled \code{Automated Review}, and still requiring human review. Build-and-test, actions and JavaScript/TypeScript analysis, CodeQL, normalization, CLA, package creation, the Azure \code{VSCode MSSQL - Pull Request} validation, and all six Linux/macOS/Windows x64/arm64 VSIX-launch jobs are green.
...
\begin{evidencebox}
High patch coverage and 675 focused passes support the implementation, but the important properties are semantic: one generation per answer, suppression when metadata authority is unavailable, no stale diagnostic publication, deterministic ranking, and strict scripting refusal. Coverage percentage alone cannot prove these; the later differential probes found ordinary constructs missing from the honesty corpus. Conversely, zero bundle-size delta proves inert packaging, not live feature quality.
\end{evidencebox}

\section{Known limitations and live review state}

\subsection{Review snapshot: 26 unresolved threads}

The live PR has 26 current, non-outdated, unresolved threads at \code{eafbbdc046}: two from Copilot and 24 from a later source-attributed Fable review posted through the PR author's account. The Fable review reports that every Tier-A reproduction was run against the compiled engine with a real catalog. This report treats those as strong review evidence, but does not relabel the reported probes as independently rerun validation.

The two Copilot threads are narrow:

\begin{enumerate}
  \item \path{sqlScripting/scriptWriter.ts} counts \code{\n} while tracking anchors but does not treat a standalone carriage return as a newline. LF and CRLF output are handled, but CR-only or mixed input can drift later anchors. Fable confirmed the reachable vector is narrow and recommended the one-line hardening.
  \item \path{sqlScripting/scriptingService.ts} says synonym \emph{targets} are not hydrated. Copilot calls that inaccurate; Fable points to the accepted \code{synonyms=lite} model where target definitions are in fact deferred. The refusal behavior is safe; wording can be tightened to name the readiness fact.
\end{enumerate}

\subsection{Tier A: ordinary-SQL trust defects}\label{sec:tiera}

The eight Tier-A findings are not architectural failures, but they are the exact user-trust failure class the diagnostic honesty design is intended to prevent. Each needs a focused fix and a zero-unexpected-diagnostic/completion-suppression corpus entry before the engine is composed into a preview consumer.

\begin{enumerate}[label=\textbf{A\arabic*.},leftmargin=2.2em,itemsep=5pt]
  \item \textbf{Trailing comments and unterminated constructs} (\path{nativeEngine.ts}). Undefined statement location falls back directly to \code{statementStart}, bypassing context suppression; a trailing comment can receive 558 items. \emph{Closure:} run suppression before fallback and add comment-boundary tests.
  \item \textbf{Alias assignment and positioned update} (\path{diagnostics.ts}). Legal \code{SELECT alias = expr} and \code{WHERE CURRENT OF cursor} names are interpreted as columns, producing false 207 diagnostics. \emph{Closure:} recognize alias assignment and \code{OF}; add honesty cases.
  \item \textbf{\code{WITHIN GROUP (ORDER BY ...)}} (diagnostics / keyword data). \code{GROUP} is treated as missing \code{BY}, producing false 102 and suppressing binder-tier analysis. \emph{Closure:} model contextual \code{WITHIN} and add aggregate cases.
  \item \textbf{\code{AT TIME ZONE}} (diagnostics / keyword data). Missing contextual \code{AT}/\code{ZONE} classification creates false 102 unit-run diagnostics. \emph{Closure:} add contextual keywords or equivalent run breakers.
  \item \textbf{Derived/VALUES alias list \code{v(a,b)}} (\path{sketch/index.ts}). The comma re-arms source parsing; \code{b} becomes a phantom table, causing false 208 and sometimes 207. \emph{Closure:} balance-skip alias column lists and test both derived forms.
  \item \textbf{Non-breaking space in pasted SQL} (\path{lexer.ts}). U+00A0 is accepted as identifier-start, can swallow the next keyword, and produces a visually baffling false 208. \emph{Closure:} treat Unicode space separators consistently as whitespace.
  \item \textbf{\code{UPDATE TOP (n) target}} (\path{sketch/index.ts}). \code{TOP} becomes the target, producing false 208 and losing \code{SET}-column completion. \emph{Closure:} apply the existing DML \code{TOP} skip to update/insert paths.
  \item \textbf{Default-schema shorthand \code{db..table}} (\path{sketch/index.ts}). Name parsing truncates at the empty schema part; the database is reported as an invalid object and alias columns disappear. \emph{Closure:} preserve the empty middle part and route to honest multipart handling.
\end{enumerate}

\subsection{Tier B/C: correctness, safety, and evidence hardening}

The other 16 Fable threads should not be lost merely because the code is source-only. They cluster into five actionable groups:

\begin{table}[H]
\centering
\small
\caption{Remaining source-attributed review findings.}
\label{tab:tierbc}
\begin{tabularx}{\textwidth}{@{}p{0.20\textwidth}p{0.48\textwidth}X@{}}
\toprule
\textbf{Group} & \textbf{Findings} & \textbf{Disposition}\\
\midrule
Parser / overlay honesty & \code{IS NOT DISTINCT FROM} clause split; GOTO labels and procedure synonyms in exec-less analysis; CREATE VIEW/module and qualified CREATE TABLE missing from overlay; opener-at-EOF token drop; GO-parity comment/corpus gap. & Fix false diagnostics and token-coverage holes before activation; define the execution-split parity owner.\\
Completion / provider & Overlay bypass for temp-table INSERT/UPDATE; case-sensitive dedupe folding; doubled qualifier in bare \code{alias.} star expansion; full scan/sort and pre-filter search cap; unknown case rule defaulting open. & Correct result omissions/edits and fail closed; benchmark/cache only with generation-safe invalidation.\\
Host correctness & Analysis cache keyed only by version and text length; synchronous features cannot be preempted by the router timer. & Include document/text identity in the cache key; document/cooperatively yield long work; extend timeout tests.\\
Scripting safety & Comment placeholders and refusal text do not consistently sanitize \code{*/} or newlines from legal pathological identifiers/type names. & Reuse the existing comment sanitizer and add adversarial fixtures.\\
Static data / integration evidence & Two \code{dm_exec_*} TVF result schemas are wrong; CI does not run features through a real \code{CatalogSnapshot}/store adapter. & Correct curated data; add one real-builder/store/provider end-to-end suite.\\
\bottomrule
\end{tabularx}
\end{table}

These findings should be resolved or explicitly dispositioned in the PR. The fully green build/test matrix proves regression stability against the existing corpus; it does not negate newly reproduced cases outside that corpus.
...
The later review materially changes the correctness disposition, however. Eight reproduced ordinary-SQL cases currently escape suppression or generate false 102/207/208 diagnostics, and the broader findings expose cache-identity, synchronous-timeout, completion-edit, provider, scripting-sanitization, and real-adapter-test gaps. Green CI shows that the existing corpus is stable; it does not close those cases. The branch remains safe for existing users only because it is not bundled or activated.
\end{takeawaybox}

\begin{statusbox}[title={Decision statement}]
Recommended disposition at this snapshot: preserve the architectural boundary, but do not call review closeout complete while 26 current threads remain open. Fix the eight Tier-A trust defects and their regression corpus before any preview consumer; triage Tier B/C with explicit PR dispositions; refresh from current \code{main}; and rerun the stated gates. Whether the inert source-only foundation merges before every Tier-B item is a maintainer choice, but it must not be exposed through a command, provider, Query Studio surface, or AI completion path until the Tier-A set, cache-identity risk, and full private-preview integration gates are closed.
\end{statusbox}
...
\midrule
PR state & \href{https://github.com/microsoft/vscode-mssql/pull/22860}{PR \#22860}, head \code{eafbbdc046}, extraction base \code{95cf790e5}, live API base \code{53b707bbe}, 26 unresolved review threads, Copilot review, source-attributed Fable review and probe report, fully green Actions/Azure rollup, package-size bot, and Codecov report.\\
