-- vectorlab_groundtruth.sql
-- Acceptance queries for the Track A synthetic corpus created by vectorlab_setup.sql.
...
--
-- Run: sqlcmd -S localhost -E -C -i vectorlab_groundtruth.sql
--
...
(
    SELECT N'01 total chunk rows' AS metric, 5000 AS expected,
           (SELECT COUNT(*) FROM dbo.VectorLabChunks) AS observed
    UNION ALL
    SELECT N'02 null vectors', 12,
           (SELECT COUNT(*) FROM dbo.VectorLabChunks
    ...
    SELECT N'06 duplicate groups (planted)', 12,
           (SELECT COUNT(DISTINCT duplicate_group) FROM dbo.VectorLabChunks
    ...
    SELECT N'18 search queries', 5,
           (SELECT COUNT(*) FROM dbo.VectorLabSearchQueries)
) AS checks
ORDER BY metric;
